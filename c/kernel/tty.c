  ...
  if (tty->ibuf_cnt < TTY_IN_BYTES)
    {
      *(tty->ibuf_head) = key;
      tty->ibuf_head++;
      if (tty->ibuf_head == tty->ibuf + TTY_IN_BYTES) tty->ibuf_head = tty->ibuf;
      tty->ibuf_cnt++;
    }
}
  ...
  while (tty->ibuf_cnt)
    {
      ...
      tty->ibuf_tail++;
      if (tty->ibuf_tail == tty->ibuf + TTY_IN_BYTES) tty->ibuf_tail = tty->ibuf;
      tty->ibuf_cnt--;
  ...
  while (i)
    {
      ...
      for (j = 0; j < bytes; j++) out_char(tty->console, buf[j]);
      i -= bytes;
      p += bytes;
    }
