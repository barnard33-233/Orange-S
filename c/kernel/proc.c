...
PUBLIC int sys_check_stack(int _unused, int _unused2, char *_unused3, struct proc *p_proc)
{
  ...
  int base = reassembly(p_proc->ldts[ss >> 3].base_high, 24, p_proc->ldts[ss >> 3].base_mid, 16,
                        p_proc->ldts[ss >> 3].base_low);
  unsigned int retaddr = *(int *)(retaddress_offset + base);

  printl("<%x>is return address\n", retaddr);

  if (retaddr > (PROC_IMAGE_SIZE_DEFAULT - PROC_ORIGIN_STACK) || retaddr > 0x10000 ||
      retaddr < 0x1000)
    {
...
PUBLIC int sys_sendrec(int function, int src_dest, MESSAGE *m, struct proc *p)
{
  ...
  assert((src_dest >= 0 && src_dest < NR_TASKS + NR_PROCS) || src_dest == ANY ||
         src_dest == INTERRUPT);
  ...
  if (copyok)
    {
      ...
      assert(p_from->p_msg);
      /* copy the message */
      phys_copy(va2la(proc2pid(p_who_wanna_recv), m), va2la(proc2pid(p_from), p_from->p_msg),
                sizeof(MESSAGE));
...
PUBLIC void dump_msg(const char *title, MESSAGE *m)
{
  ...
  printl(
      ...
      title, (int)m, packed ? "" : "\n        ", proc_table[m->source].name, m->source,
      packed ? " " : "\n        ", m->type, packed ? " " : "\n        ", m->u.m3.m3i1, m->u.m3.m3i2,
      m->u.m3.m3i3, m->u.m3.m3i4, (int)m->u.m3.m3p1, (int)m->u.m3.m3p2, packed ? "" : "\n",
      packed ? "" : "\n" /* , */
  );
}
