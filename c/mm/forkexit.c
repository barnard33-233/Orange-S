...
PUBLIC int do_fork()
{
  ...
  *p = proc_table[pid];
  ...
  assert((caller_T_base == caller_D_S_base) && (caller_T_limit == caller_D_S_limit) &&
  ...
  /* child's LDT */
  init_desc(&p->ldts[INDEX_LDT_C], child_base, (PROC_IMAGE_SIZE_DEFAULT - 1) >> LIMIT_4K_SHIFT,
            DA_LIMIT_4K | DA_32 | DA_C | PRIVILEGE_USER << 5);
  init_desc(&p->ldts[INDEX_LDT_RW], child_base, (PROC_IMAGE_SIZE_DEFAULT - 1) >> LIMIT_4K_SHIFT,
