...
PUBLIC int do_exec()
{
  ...
  phys_copy((void *)va2la(TASK_MM, pathname), (void *)va2la(src, mm_msg.PATHNAME), name_len);
      ...
      if (prog_hdr->p_type == PT_LOAD)
        {
          ...
          phys_copy((void *)va2la(src, (void *)prog_hdr->p_vaddr),
                    (void *)va2la(TASK_MM, mmbuf + prog_hdr->p_offset), prog_hdr->p_filesz);
        }
    }
  ...
  phys_copy((void *)va2la(TASK_MM, stackcopy), (void *)va2la(src, mm_msg.BUF), orig_stack_len);
  ...
  if (orig_stack_len)
    { /* has args */
      ...
      for (; *q != 0; q++, argc++) *q += delta;
    }

  phys_copy((void *)va2la(src, orig_stack), (void *)va2la(TASK_MM, stackcopy), orig_stack_len);
