...
PRIVATE void init_fs()
{
  ...
  for (; sb < &super_block[NR_SUPER_BLOCK]; sb++) sb->sb_dev = NO_DEV;
...
PRIVATE void mkfs()
{
  ...
  for (i = 0; i < (NR_CONSOLES + 3); i++) fsbuf[0] |= 1 << i;
  ...
  memset(fsbuf, 0, SECTOR_SIZE);
  for (i = 1; i < sb.nr_smap_sects; i++) WR_SECT(ROOT_DEV, 2 + sb.nr_imap_sects + i);
  ...
  int bit_offset =
      INSTALL_START_SECT - sb.n_1st_sect + 1; /* sect M <-> bit (M - sb.n_1stsect + 1) */
...
PUBLIC struct inode *get_inode(int dev, int num)
{
  ...
  int blk_nr =
      1 + 1 + sb->nr_imap_sects + sb->nr_smap_sects + ((num - 1) / (SECTOR_SIZE / INODE_SIZE));
  RD_SECT(dev, blk_nr);
  struct inode *pinode =
...
PUBLIC void sync_inode(struct inode *p)
{
  ...
  int blk_nr =
      1 + 1 + sb->nr_imap_sects + sb->nr_smap_sects + ((p->i_num - 1) / (SECTOR_SIZE / INODE_SIZE));
  RD_SECT(p->i_dev, blk_nr);
  pinode =
      (struct inode *)((u8 *)fsbuf + (((p->i_num - 1) % (SECTOR_SIZE / INODE_SIZE)) * INODE_SIZE));
      ...
      if (p->filp[i])
        {
          ...
          if (--p->filp[i]->fd_cnt == 0) p->filp[i]->fd_inode = 0;
          p->filp[i] = 0;
        }
    }
  return 0;
