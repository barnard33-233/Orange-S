...
#define DISKLOG_RD_SECT(dev, sect_nr)                                                              \
  rw_sector(DEV_READ, dev, (sect_nr)*SECTOR_SIZE, SECTOR_SIZE, /* read one sector */               \
            getpid(), logdiskbuf);
#define DISKLOG_WR_SECT(dev, sect_nr)                                                              \
  rw_sector(DEV_WRITE, dev, (sect_nr)*SECTOR_SIZE, SECTOR_SIZE, /* write one sector */             \
            getpid(), logdiskbuf);
...
PUBLIC int disklog(char *logstr)
{
  ...
  static int pos = 0;
  if (!pos)
    { /* first time invoking this routine */
      ...
      if (sects_left != 0) panic("sects_left should be 0, current: %d.", sects_left);
...
PUBLIC void dump_fd_graph(const char *fmt, ...)
{
  ...
  for (i = 0; i < NR_TASKS + NR_PROCS; i++, p_proc++)
    {
      if (p_proc->p_flags == FREE_SLOT) continue;
      if ((i == TASK_TTY) || (i == TASK_SYS) || (i == TASK_HD) ||
          /* (i == TASK_FS)  || */
          (i == callerpid))
        continue;
  ...
  /* head */
  logbufpos += sprintf(logbuf + logbufpos, "digraph filedesc%02d {\n", graph_idx++);
  ...
  for (i = 0; i < NR_TASKS + NR_PROCS; i++, p_proc++)
    {
      ...
      logbufpos += sprintf(logbuf + logbufpos,
                           ...
                           "|<f2> p_parent:%d%s"
                           "|<f3> eip:0x%x",
                           i, p_proc->name, i, p_proc->p_flags, proc_flags[p_proc->p_flags],
                           p_proc->p_parent, p_proc->p_parent == NO_TASK ? "(NO_TASK)" : "",
                           p_proc->regs.eip);

      int fnr = 3;
      for (k = 0; k < NR_FILES; k++)
        {
          if (p_proc->filp[k] == 0) continue;

          int fdesc_tbl_idx = p_proc->filp[k] - f_desc_table;
          logbufpos += sprintf(logbuf + logbufpos, "\t|<f%d> filp[%d]: %d", fnr, k, fdesc_tbl_idx);
          pfm[pfm_idx].pid = i;
  ...
  for (i = 0; i < NR_FILE_DESC; i++)
    {
      if (f_desc_table[i].fd_inode == 0) continue;
      ...
      logbufpos += sprintf(logbuf + logbufpos,
                           ...
                           "|<f3> fd_cnt:%d"
                           "|<f4> fd_inode:%d",
                           i, f_desc_table[i].fd_mode, f_desc_table[i].fd_pos,
                           f_desc_table[i].fd_cnt, inode_tbl_idx);
      fim[fim_idx].desc = i;
  ...
  for (i = 0; i < NR_INODE; i++)
    {
      if (inode_table[i].i_cnt == 0) continue;
      ...
      logbufpos += sprintf(logbuf + logbufpos,
                           ...
                           "|<f6> i_cnt:%d"
                           "|<f7> i_num:%d",
                           i, inode_table[i].i_mode, inode_table[i].i_size,
                           inode_table[i].i_start_sect, inode_table[i].i_nr_sects,
                           inode_table[i].i_dev, inode_table[i].i_cnt, inode_table[i].i_num);
  ...
  logbufpos +=
      sprintf(logbuf + logbufpos, "\t\tlabel = \"sector map (dev size: %xh)\";\n", sb->nr_sects);
  logbufpos += sprintf(logbuf + logbufpos, "\t}\n");
#endif
  ...
  for (i = 0; i < SECTOR_SIZE / sizeof(struct inode); i++, p += INODE_SIZE)
    {
      ...
      if (pinode->i_start_sect == 0) continue;
      int start_sect;
      int end_sect;
      if (pinode->i_mode)
        {
          ...
          logbufpos += sprintf(logbuf + logbufpos,
                               ...
                               "|<f2> i_size:0x%x"
                               "|<f3> sect: %xh-%xh",
                               i + 1, pinode->i_size, start_sect, end_sect);
  ...
  for (i = 0; i < pfm_idx; i++)
    {
      logbufpos += sprintf(logbuf + logbufpos, "\t\"proc%d\":f%d -> \"filedesc%d\":f3;\n",
                           pfm[i].pid, pfm[i].filp, pfm[i].desc);
    }
#endif
